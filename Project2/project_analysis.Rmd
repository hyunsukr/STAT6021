...
sample<-sample.int(nrow(first_transformation_full), floor(.50*nrow(first_transformation_full)), replace = F)
train<- first_transformation_full[sample, ] ##training data frame
test<-first_transformation_full[-sample, ] ##test data frame
...
anova(without_inter, interaction1)
interaction2 <- lm( charges ~ (bmi) * smoker + children  + region + age, data = dropped_sign)
anova(without_inter, interaction2)
```

See which is interaction

```{r}
transform <- data[1:7]
transform$charges <- dropped_sign$charges^0.15
reduced <- lm(charges ~ age + sex + bmi + children + region + smoker, data = transform)
bmi_smoker <- lm(charges ~ age + sex + bmi*smoker + children + region, data=transform)
anova(reduced, bmi_smoker)
bmi_smoker_age <- lm(charges ~ age*smoker + sex + bmi*smoker + children + region, data=transform)
anova(bmi_smoker, bmi_smoker_age)
```

```{r}
summary(bmi_smoker_age)
```

```{r}
yhat_inter <- bmi_smoker_age$fitted.values
res_inter <- bmi_smoker_age$residuals
data %>%
  ggplot(aes(yhat_inter, res_inter)) + 
  geom_point() + 
  theme_bw() +
  geom_hline(yintercept = 0, color="red")
```
...
```{r}
{
  qqnorm(bmi_smoker_age$residuals)
  qqline(bmi_smoker_age$residuals, col="red")
}
```
